...
class TokenService {
  ...
  _saveStorage() {
    try {
      ...
      fs.writeFileSync(tmpPath, JSON.stringify(this.tokens, null, 2), 'utf-8');
  ...
  init() {
    this._ensureStorage();
    // If GATEWAY_API_KEY is configured in env and store is empty, seed it as a default token
    if (this.tokens.length === 0 && ENV.GATEWAY_API_KEY) {
      const envKeys = ENV.GATEWAY_API_KEY.split(',').map(k => k.trim()).filter(Boolean);
      for (const [idx, key] of envKeys.entries()) {
        this.tokens.push({
          id: `env-${idx + 1}`,
          name: `Env Key #${idx + 1}`,
          token: key,
  ...
  validateToken(clientKey) {
    ...
    const envKeys = ENV.GATEWAY_API_KEY
      ? ENV.GATEWAY_API_KEY.split(',').map(k => k.trim()).filter(Boolean)
      : [];

    // If no keys configured in env and no tokens generated, gateway is in open dev mode
    const hasAnyProtection = envKeys.length > 0 || this.tokens.some(t => t.status === 'active');
    if (!hasAnyProtection) {
      return { valid: true, openMode: true };
    }
    ...
    // 1. Check against active tokens in token store with timing-safe comparison
    ...
    // 2. Check against envKeys
    for (const envKey of envKeys) {
      if (this.safeCompare(envKey, clientKey)) {
  ...
  hasKeys() {
    ...
    const envKeys = ENV.GATEWAY_API_KEY
      ? ENV.GATEWAY_API_KEY.split(',').map(k => k.trim()).filter(Boolean)
      : [];
    return envKeys.length > 0 || this.tokens.some(t => t.status === 'active');
  }
}
